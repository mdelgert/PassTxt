...
void GfxHandler::printMessage(const String &message, int x, int y, int textSize)
{
    ...
    if (x < 0)
        x = 5; // Default X position
    if (y < 0)
        y = 5; // Default Y position

    tft.setTextSize(textSize);
    tft.setCursor(x, y);
    tft.println(message); // String works directly with println()
}
...
void GfxHandler::registerCommands()
{
    CommandHandler::registerCommand("tft", [](const String &command)
       ...
       if (cmd == "print") {
            ...
            // Extract message, x, y, and size
            int firstComma = args.indexOf(',');
            int secondComma = args.indexOf(',', firstComma + 1);
            int thirdComma = args.indexOf(',', secondComma + 1);

            String message = args;
            int x = -1, y = -1, size = -1;  // Default to negative to use defaults
            
            if (firstComma != -1) {
                message = args.substring(0, firstComma);
                x = args.substring(firstComma + 1, secondComma != -1 ? secondComma : args.length()).toInt();
                if (secondComma != -1) {
                    y = args.substring(secondComma + 1, thirdComma != -1 ? thirdComma : args.length()).toInt();
                }
                if (thirdComma != -1) {
                    size = args.substring(thirdComma + 1).toInt();
                }
            }

            printMessage(message.c_str(), x, y, size);
        }
            ...
            debugW("Unknown tft subcommand: %s", cmd.c_str());
                                         ...
                                         "  Subcommands:\n"
                                         "  print <message>[,<x>,<y>,<size>] - Print a message to TFT screen at optional x,y,size\n"
                                         "  clock <true|false> - Show or hide current time on tft screen\n"
                                         "  draw <image>,<x>,<y>,<width>,<height> - Draw an image on the screen");
}

#endif // USE_GFX_HANDLER

// tft print test
// tft print test,30,20,4
// tft clock true
// tft clock false
